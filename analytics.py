...
def get_match_id(team1, team2):
    ...
    for element in responce_json:
        try:
            if (
                (
                    element["team_name_radiant"] == team1
                    and element["team_name_dire"] == team2
                )
                and element["deactivate_time"] == 0
            ) or (
                (
                    element["team_name_radiant"] == team2
                    and element["team_name_dire"] == team1
                )
                and element["deactivate_time"] == 0
            ):
...
def get_match_state(match_id):
    responce = requests.get(
        "https://api.steampowered.com/IDOTA2Match_570/GetLiveLeagueGames/v001?key=FB73DF3B5529D699D2AC8CBE7F861B72"
    )
    responce_json = json.loads(responce.text)
    for element in responce_json["result"]["games"]:
        if element["match_id"] == match_id:
            return (
                (
                    f'{element["scoreboard"]["dire"]["barracks_state"]:08b}',
                    f'{element["scoreboard"]["dire"]["tower_state"]:08b}',
                ),
                (
                    f'{element["scoreboard"]["radiant"]["barracks_state"]:08b}',
                    f'{element["scoreboard"]["radiant"]["tower_state"]:08b}',
                ),
            )
